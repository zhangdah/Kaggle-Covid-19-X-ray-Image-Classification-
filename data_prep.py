...
if __name__ == '__main__':
    ...
    for img, label in iter(train_loader):
        ...
        image_augmentation = transforms = torch.nn.Sequential(
            torchvision.transforms.RandomCrop((torch.randint(low=224, high=256, size=(2,)))),
            torchvision.transforms.Resize((224, 224)),
        )
        image = image_augmentation(img)
        image = image * (1.1 - torch.rand(1)/5)

    	# get the save directory
        directory = "Data/Training/"
        if (label.item() == 0):
            directory += "covid/"
            torchvision.utils.save_image(image, directory+"covid"+str(covid_counter)+".png", format ="png");
            covid_counter += 1

        elif (label.item() == 1):
            directory += "healthy/"
            torchvision.utils.save_image(image, directory+"healthy"+str(healthy_counter)+".png", format ="png");
            healthy_counter += 1

        else:
            directory += "viral/"
            torchvision.utils.save_image(image, directory+"viral"+str(viral_counter)+".png", format ="png");
            viral_counter += 1
